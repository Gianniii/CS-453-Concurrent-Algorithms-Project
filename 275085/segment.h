 ...
 * //THIS IS AN OPTIMIZATION COULD BE REPLACE BY STACK
 * //OR NOT NECESSARY AT ALL!!
 **/
typedef struct {
  size_t n_words;
  void *cp0;        // first copy from description
  void *cp1;        // second copy from description
  ...
  int *index_modified_words;     // array to store index of accessed words
  _Atomic(int) num_writen_words; // can be used for next to keep track of next
                                 // free index in *index_modified_words array
} segment_t;
